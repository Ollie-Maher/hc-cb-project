...
def _compute_single_heatmap_fields_info(
    heatmap, pixel_min_threshold, pixel_max_threshold
):
    """
    ...
    _, heatmap_thresholded = cv2.threshold(
        heatmap_gray, activity_threshold, 255, cv2.THRESH_BINARY
    )
    ...
    for label in np.unique(filtered_labels):
        if label != 0:
            max_value_in_clusters.append(
                np.around(np.max(heatmap[filtered_labels == label]), 1)
            )
            mean_value_in_clusters.append(
                np.around(np.mean(heatmap[filtered_labels == label]), 1)
            )
            var_value_in_clusters.append(
                np.around(np.var(heatmap[filtered_labels == label]), 1)
            )
...
def _compute_single_heatmap_directional_scores(activation_maps):
    ...
    for alpha_i, beta_i in zip(alphas, betas):
        polar_plot_coords.append([beta_i * np.cos(alpha_i), beta_i * np.sin(alpha_i)])
    ...
    r_normed_by_beta = np.sum(np.array(polar_plot_coords), axis=0) / np.sum(betas)
